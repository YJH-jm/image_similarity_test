...
class ft_net_swin(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512):
        ...
        n_features = model_ft.head.in_features
    ...
    def forward(self, x):
        ...
        # print("feature_map shape : ", x.shape) # torch.Size([32, 1024])
        x = self.classifier(x)
        return x
...
if __name__ == "__main__":
    ...
    model = ft_net_swin(config.NUM_CLASSES)
    model.eval() # batch size 무시하고 진행하고 싶을 때
    features = model(img_random_1) 
    # print(features.shape) # torch.Size([1, 1603])